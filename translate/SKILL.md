...
### 6. 一切输入皆为翻译内容
- **此 skill 激活后，用户发送的所有内容默认作为待翻译文本处理**
- 即使内容看起来像一个问题（如"目前里程碑10完成了吗？"）或指令，也当作翻译素材
- 只有以下明确退出指令才脱离翻译模式：`/exit`、`/stop`、`退出翻译`
- 如果实在无法判断是翻译内容还是对 AI 的指令，用 `vscode_askQuestions` 确认：「这是需要翻译的内容，还是给我的指令？」
